"""
Problem 11 - Chromatic Number (Trying k = 1, 2, 3, ...)
======================================================

Implement:
  - chromatic_number(graph): smallest k >= 0 for which `graph` has a proper
    k-coloring (use k_coloring from Problem 10). A graph with vertices but no
    edges needs 1 color; the empty graph on 0 vertices needs 0.
  - is_three_colorable(graph): True iff chromatic_number(graph) <= 3.

`k_coloring` (from Problem 10) is re-provided so this file is self-contained.
...
def k_coloring(graph: Graph, k: int) -> Optional[List[int]]:
    """Backtracking search for a proper k-coloring, or None (from Problem 10)."""
    n, edges = graph
    if n == 0:
        return []
    if k <= 0:
